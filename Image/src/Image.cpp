...
ImageType Image::get_file_type(const char* filename) {
	...
	//this function returns the address of a pixel value when we consider that's it is à grid 
  uint8_t& Image::at(int j, int i){
	  if(channels!=1){
		  printf("please make sure that the image only have one channel");
		  return data[0];
	  }else{
		return data[(j*w+i)*channels];
	  }
